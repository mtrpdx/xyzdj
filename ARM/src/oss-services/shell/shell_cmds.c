...
const char shell_help_browse[] = "[drive]\n"
    "  drive to load - eg. `sf:` for flash memory or `sd:` for SD card.\n";

const char shell_help_summary_browse[] = "Loads file browser.\n";
...
/* static void browse_state(SHELL_CONTEXT *ctx, char *crtname, MUSIC_FILE *mf) */
/* { */
/*     // Rather than using pointer to MUSIC_FILE, I need to point to array of */
/*     // MUSIC_FILEs */
/*     printf("%s", "--------------------\n"); */
/*     for (int i=0; i<10; i++) { */
/*         printf("| %16s |\n", mf->fname); */
...
/* static void print_frame(SHELL_CONTEXT *ctx, char *files[]) { */
/*     // Prints the frame showing available files to console */
/*     // input: char pointer to `files`, which is a list of strings */
/*     // containing filtered filenames */
/*     printf("%.*s", 20, "-----"); */
/*     while (*files) */
/*     { */
/*         printf("\n | %16s |", *files); */
...
/* void get_files_to_display(SHELL_CONTEXT *ctx, const char *crtname, char *files_to_display[], uint32_t max) */
/* { */
/*     void *d; */
/*     uint32_t nFilesFound = 0; */
/*     FS_DEVMAN_DIRENT *ent; */
/*     // TODO: Add support for more extensions */
/*     // Currently supported extensions: 'wav' */
/*     /\* const int n_valid_exts = 1; *\/ */
/*     const char valid_file_extensions[][4] = {"wav"}; */
...
/*     if ((d = fs_devman_opendir(crtname)) != NULL) */
/*     { */
/*         /\* while (shell_browse_getchar(0, ctx->usr) != 'q') { *\/ */
/*         nFilesFound = 0; */
/*         /\* printf("%s", crtname); *\/ */

/*         // Walking through files on device */
/*         while (((ent = fs_devman_readdir(d)) != NULL) && */
/*                (nFilesFound < max)) */
/*         { */
/*             if ((ent->fname)[0] != '.') { */
...
/*                     /\*     *phasdirs = 1; *\/ */
/*                     /\* printf(" %s", ent->fname); *\/ */
/*                     /\* files_to_display[nFilesFound] = ent->fname; *\/ */
/*                     /\* nFilesFound++; *\/ */
/*                 } else { */
/*                     char *curr_ext = get_file_extension(ent->fname); */
/*                     printf("evaluating file extension: "); */
/*                     printf(" %16s", ent->fname); */
/*                     printf(" %6s\n", curr_ext); */
/*                     for (int i = 0; i < 1; i++) { */
/*                         if (strcmp(curr_ext, valid_file_extensions[i]) == 0) { */
/*                             /\* printf("\n"); *\/ */
/*                             /\* printf(" %s ", curr_ext); *\/ */
/*                             /\* printf(" %s", ent->fname); *\/ */
/*                             printf("File extension matched valid file extensions.\n"); */
/*                             printf(" %16s", ent->fname); */
/*                             printf(" %6s\n", curr_ext); */
/*                             files_to_display[nFilesFound] = ent->fname; */
/*                             printf("%s\n",*files_to_display[nFilesFound]); */
/*                             nFilesFound++; */
/*                             } */
/*                     } */
/*                     SHELL_FREE(curr_ext); */
...
static void shell_browse_helper(
    SHELL_CONTEXT *ctx,
    const char *crtname,
    int recursive,
    int *phasdirs,
    ...
    /* int xDisplayChars = 40; // Number of chars to display in x-dir */
    /* int yDisplayChars = 10; // " y-dir, max files to display */

    /* char (*fnames)[128]; */
    /* fnames = SHELL_MALLOC(10 * sizeof(*fnames)); */
    ...
    /* for (int i = 0; i < 10; i++) { */
    /*     strcpy(fnames[i],"empty"); */
    /* } */

    /* /\* char *files_to_display[max_files_to_display]; // array of pointers to filenames *\/ */
    /* /\* for (int i = 0; i < max_files_to_display; i++) { *\/ */
    /* /\*     files_to_display[i] = "empty"; *\/ */
    /* /\* } *\/ */
    /* /\* for (int j = 0; j < max_files_to_display; j++) { *\/ */
    /* /\*     printf("%s\n", files_to_display[j]); *\/ */
    /* /\*     // This prints correctly. Need to figure out proper way of passing pointers *\/ */
    /* /\* } *\/ */

    /* /\* int input_key; // can store single char *\/ */
    ...
    /* // FIXME */
    /* /\* const char *fname; *\/ */
    /* /\* get_files_to_display(ctx, crtname, files_to_display, max_files_to_display); *\/ */

    /* /\* print_frame(ctx, files_to_display); *\/ */
    ...
    /*     // Walking through files on device */
    /*     while (((ent = fs_devman_readdir(d)) != NULL) && */
    /*            (nFilesFound < yDisplayChars)) */
    /*     { */
    /*         // FIXME */
    /*         fname = ent->fname; // This is wrong */
    ...
    /*                 /\* nFilesFound++; *\/ */
    /*             } else { */
    /*                 char *curr_ext = get_file_extension(fname); */
    /*                 /\* printf("evaluating file extension: "); *\/ */
    /*                 for (int i = 0; i < 1; i++) { */
    /*                     if (strcmp(curr_ext, valid_file_extensions[i]) == 0) { */
    /*                         printf(" %16s", fname); */
    /*                         printf(" %6s\n", curr_ext); */
    /*                         strcpy(fnames[nFilesFound], fname); */
    /*                         /\* printf("\n"); *\/ */
    /*                         /\* printf(" %s ", curr_ext); *\/ */
    /*                         /\* printf(" %s", ent->fname); *\/ */
    /*                         /\* printf("File extension matched valid file extensions.\n"); *\/ */
    ...
    /*                         /\* printf("%s\n",*files_to_display[nFilesFound]); *\/ */
    /*                         /\* strcpy(fext, curr_ext); *\/ */


    /*                         /\* if (fname) { *\/ */
    /*                         /\*     if (mf->fname) { *\/ */
    /*                         /\*         BROWSE_FREE(mf->fname); mf->fname = NULL; *\/ */
    /*                         /\*     } *\/ */
    /*                         /\* mf->fname = BROWSE_MALLOC(strlen(fname)+1); *\/ */
    ...
    ok = browse_main(argc, argv, ctx, crtname);
    if (ok < 0) {
        printf("Error\n");
    }

    /* browse_state(ctx, crtname, mfs); */
}

void shell_browse(SHELL_CONTEXT *ctx, int argc, char **argv)
{
    FS_DEVMAN_RESULT result;
    int phasdirs;
    char *device;
    ...
    shell_browse_helper(ctx, device, 0, &phasdirs, argc, argv);


    /* printf( "up/down arrows to move, Enter loads a file.\n"); */
